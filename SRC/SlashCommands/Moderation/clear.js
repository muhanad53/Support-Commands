...
const { Client, SlashCommandBuilder, CommandInteraction, PermissionFlagsBits, MessageContextMenuCommandInteraction, CommandInteractionOptionResolver, MessageType } = require('discord.js')
    ...
    async execute(interaction, client) {
        ...
        if (Target) {
            ...
            await channel.bulkDelete(Amount, true).then(async messages => {
                Response.setDescription(`🗑 **Cleared** \`${messages.size}\` **from this channel :** <#${channel.id}>.`)
                await interaction.reply({embeds: [Response], ephemeral: true})
            })
        }
    }
